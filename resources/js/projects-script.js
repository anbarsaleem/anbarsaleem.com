...
function requestUserRepoInfo() {
  ...
  const REPO_LIST_ENDPOINT = "/users/anbarsaleem/repos";
  
  fetch(GITHUB_API_BASE_URL + REPO_LIST_ENDPOINT)
  	.then(res => res.json())
    .then(projects => {
    	projects.map((p, pNum) => {
      	const LANGUAGES_FOR_PROJECT_ENDPOINT = `/repos/anbarsaleem/${p.name}/languages`;
...
function generateProjectField(project, number) {
  ...
  for (let i in project.languages) {
    ...
    var specific_language_div_classes = " project_tag_" + project.languages[i].toString().toLowerCase();
  ...
  project_content_div.append(project_title_div, description_p, project_details_div);
